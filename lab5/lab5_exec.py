...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    move_arm(pub_cmd, loop_rate, lab_invk(float(target_xw_yw_zw[0]), float(target_xw_yw_zw[1]), 0.03, 0), vel, accel)
...
def main():
    ...
    """
    #Target position for green
    LOC_GREEN = np.array([[.28656, -.25217, 0.03], [.28029, -.30698, 0.03], [.34011,-.24713, 0.03], [.33884, -.30511, 0.03]])
    #Change coordinates to the world  frame
    LOC_GREEN = LOC_GREEN + np.array([-0.15,+0.15,0.0])
    #Target position for yellow
    LOC_YELLOW = np.array([[.40122,-.31005, 0.03],[.45805, -.25346, 0.03], [.39672, -.24599, 0.03], [.46414,-.31507,0.03]])
    #Change coordinates to the world  frame
    LOC_YELLOW = LOC_YELLOW + np.array([-0.15,+0.15, 0.0])
    print(LOC_YELLOW)
    print("green"+str(xw_yw_G))
    print("yellow"+str(xw_yw_Y))
    ...
    if(len(xw_yw_G) > 1) : 
        move_block(pub_command, loop_rate, [xw_yw_G[1][0], xw_yw_G[1][1], .025], LOC_GREEN[1], vel, accel)
    if(len(xw_yw_G) > 0) : 
        move_block(pub_command, loop_rate, [xw_yw_G[0][0], xw_yw_G[0][1], .025], LOC_GREEN[0], vel, accel)
    if(len(xw_yw_Y) > 1) : 
        move_block(pub_command, loop_rate, [xw_yw_Y[1][0], xw_yw_Y[1][1], .025], LOC_YELLOW[2], vel, accel)
    if(len(xw_yw_Y) > 0) : 
        move_block(pub_command, loop_rate, [xw_yw_Y[0][0], xw_yw_Y[0][1], .025], LOC_YELLOW[1], vel, accel)
    # # ========================= Student's code ends here ===========================
